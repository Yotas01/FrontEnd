...
export class RaeComponent implements OnInit {
  ...
  raes: RAE[] = RAES.map(data => new RAE(data.id,data.description,data.assessmentTools
    .map(data => new AssessmentTool(data.description,data.value,data.totalStudents,data.performanceIndicators
      .map(data => new PerformanceIndicator(data.description,data.percentage,data.exemplary,data.competent,data.below))))));
